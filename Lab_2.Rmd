...
Joyce: If the binwidth is small then we would see more details in the histograms. Comparing these three histograms, the binwidth = 15 has more details than the binwidth = 150 but the binwidth = 30 would be the better histograms to present to readers. 
...
Joyce:    
Mean - average dep_delay, counting each dep_delay but if there is huge dep_delay time(outliner) which would effect the outcome. 
Medium - it takes middle value which would not effect by huge dep_delay time (outliner) but can not really give me a good picture of finding a month to travel. 
...
Joyce: LGA due to the higher ontime departure rate than other 2 airports.
...
dl_aa_ua_repl <- nycflights %>%
  filter(carrier == "AA" | carrier == "DL" | carrier == "UA")
ggplot(data = dl_aa_ua_repl, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  geom_point()

```
 Joyce: Based on the graph, I can still see if a point (x,y) ~ (60~65, 0) which means a flight departure delay 60 to 65 minutes, the flight still could make it on time. Cut off time should between 60~65. 
